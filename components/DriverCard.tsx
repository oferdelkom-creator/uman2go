import { getLocale, getTranslations } from "next-intl/server";
import { Link } from "@/i18n/navigation";
import { CardImage } from "@/components/ui/CardImage";
import { DefaultCardImage } from "@/components/ui/DefaultCardImage";
import { tField } from "@/lib/i18n-content";
import type { Driver } from "@/lib/types";

export async function DriverCard({ driver }: { driver: Driver }) {
  const [t, locale] = await Promise.all([getTranslations(), getLocale()]);
  const photo = driver.photos[0];
  const vehicleType = tField(driver.vehicle_type, driver.vehicle_type_i18n, locale);

  return (
    <Link
      href={`/transport/${driver.slug}`}
      className="group flex h-full flex-col overflow-hidden rounded-3xl bg-white shadow-lg shadow-brand-navy/10 ring-1 ring-brand-navy/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-brand-cream-deep">
        {photo ? <CardImage src={photo} alt={driver.name} /> : <DefaultCardImage kind="car" />}
      </div>
      <div className="flex flex-1 flex-col p-4">
        <h3 className="font-display text-lg font-bold text-brand-navy">{driver.name}</h3>
        <p className="mt-1 text-sm text-foreground/60">{vehicleType || t("transport.list.defaultVehicleType")}</p>
        <span className="mt-4 block rounded-full bg-brand-navy px-4 py-2.5 text-center text-sm font-semibold text-white transition-colors group-hover:bg-brand-terracotta">
          {t("transport.list.viewAndRequest")}
        </span>
      </div>
    </Link>
  );
}
